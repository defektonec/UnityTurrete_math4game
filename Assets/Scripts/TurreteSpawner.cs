...
public class TurreteSpawner : MonoBehaviour
{
    ...
    [SerializeField] private GameObject turreteInstance;
    ...
    void Update()
    {
        if (Physics.Raycast(transform.position, transform.forward, out hit, distance, layers))
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && angle <= maxSlope)
            {
                Quaternion rotation = Quaternion.LookRotation(directionZ, directionY);
                GameObject.Instantiate(turreteInstance, hit.point, rotation);
            }
        }
    }
